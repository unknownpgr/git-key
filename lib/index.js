...
async function hide(encryptedFileName = SECRET_ENCRYPTED_FILE) {
    ...
    await promises_1.default.writeFile(encryptedFileName, encryptedFile, "utf-8");
    let encodedPassword = password.toString("base64");
    if (encryptedFileName !== SECRET_ENCRYPTED_FILE) {
        const encodedFileName = Buffer.from(encryptedFileName, "utf-8").toString("base64");
...
async function reveal(password) {
    ...
    if (!encodedFileName) {
        ...
        encryptedFileName = Buffer.from(encodedFileName, "base64").toString("utf-8");
    }
    let encryptedFile;
    try {
        encryptedFile = await promises_1.default.readFile(encryptedFileName, {
            encoding: "utf-8",
        });
    }
    catch {
...
async function cli() {
    ...
    if (argv.v || argv.verbose || process.env.VERBOSE)
        verbose = true;
    const encryptedFileName = (argv.n || argv.filename) ||
